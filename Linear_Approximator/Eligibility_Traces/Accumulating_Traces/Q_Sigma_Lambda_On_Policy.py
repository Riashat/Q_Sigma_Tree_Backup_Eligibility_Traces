...
def q_sigma_lambda_on_policy_static_sigma(env, theta, num_episodes, discount_factor=1.0, epsilon=0.1, epsilon_decay=1.0, alpha=0.5, lambda_param=0.1):
	...
	for i_episode in range(num_episodes):
		...
		print "Episode Number, Q (sigma, lambda) On Policy, Static Sigma:", i_episode
...
def q_sigma_lambda_on_policy_dynamic_sigma(env, theta, num_episodes, discount_factor=1.0, epsilon=0.1, epsilon_decay=1.0, alpha=0.5, lambda_param=0.1):
...
def main():
	...
	stats_sarsa_tb_lambda, cumulative_errors = q_sigma_lambda_on_policy_dynamic_sigma(env, theta, num_episodes)
	rewards_smoothed_stats_tb_lambda = pd.Series(stats_sarsa_tb_lambda.episode_rewards).rolling(smoothing_window, min_periods=smoothing_window).mean()	
	cum_rwd = rewards_smoothed_stats_tb_lambda
	cum_err = cumulative_errors
	np.save('/Users/Riashat/Documents/PhD_Research/BASIC_ALGORITHMS/My_Implementations/Project_652/Code/Linear_Approximator/Eligibility_Traces/Accumulating_Traces/WindyGrid_Results/'  + 'Q_sigma_lambda_OnPolicy_Dynamic_Sigma_RBF_Cum_Rwd_2' + '.npy', cum_rwd)
	np.save('/Users/Riashat/Documents/PhD_Research/BASIC_ALGORITHMS/My_Implementations/Project_652/Code/Linear_Approximator/Eligibility_Traces/Accumulating_Traces/WindyGrid_Results/'  + 'Q_sigma_lambda_OnPolicy_Dynamic_Sigma_RBF_Cum_Err_2' + '.npy', cum_err)
	plotting.plot_episode_stats(stats_sarsa_tb_lambda)
	env.close()
